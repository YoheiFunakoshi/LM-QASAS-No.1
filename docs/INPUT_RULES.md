...
対象は`src/lmqasas/inputs.py`の`input-v1`です。1人分のPre・Peak・Postを読み取り、採用した行から各時点のcloneを作ります。元CSVは変更しません。以下は**現在の実装条件**であり、元論文の前処理を完全に再現できたことを意味しません。
