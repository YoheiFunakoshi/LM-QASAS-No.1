...
**環境構築とAbLang2の人工配列10本によるCPU動作確認が完了しました（2026-09-19）。重複CDR-H3をまとめて指定数を選ぶ部品も実装しました。実データのembedding、入力・スコア計算、可視化、GUIは未実装です。**
...
公式モデルをローカルに取得し、人工配列から10×480のembeddingを生成できました。APIキーは不要です。論文と同じpooling条件かは未確定で、候補抽出の性能検証はまだ行っていません。
