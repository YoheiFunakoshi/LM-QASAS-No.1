"""Render private UMAP min_dist comparisons from frozen numerical artifacts.
...
def _checked_inputs(folder, reference, font_file):
    ...
    seeds, min_dists = info.get("seeds"), info.get("min_dists")
    if (not isinstance(seeds, list) or len(seeds) != 2 or len(set(seeds)) != 2
            ...
            or min_dists != [0.1, 0.3, 0.5]):
        raise ValueError("Exactly two valid seeds and the predeclared min_dist series are required.")
    ...
    expected_keys = {f"md{number}_seed{seed}" for number in min_dists for seed in seeds}
    fits = info.get("fits")
    if not isinstance(fits, dict) or set(fits) != expected_keys:
        raise ValueError("Every combination of the min_dist series and both seeds is required.")
    ...
    for number in min_dists:
        for seed in seeds:
            key = f"md{number}_seed{seed}"
            fit = fits[key]
            if not isinstance(fit, dict) or fit.get("seed") != seed or fit.get("min_dist") != number:
                raise ValueError("A fit does not match its declared min_dist and seed.")
            parameters = fit.get("parameters")
            if (not isinstance(parameters, dict) or parameters.get("min_dist") != number
            ...
            remaining = {name: value for name, value in parameters.items()
                         if name not in ("min_dist", "random_state", "transform_seed")}
            if fixed_parameters is None:
                fixed_parameters = remaining
            elif fixed_parameters != remaining:
                raise ValueError("UMAP settings other than min_dist and seed must remain fixed.")
...
def _figure(info, grids, reference_pixels, font, seed, vmax):
    ...
    shared._label(figure, font, 0.045, 0.977, "UMAPのmin_distによる背景分布の比較", 21, weight="bold")
    shared._label(figure, font, 0.045, 0.955,
                  f"seed = {seed}  ／  現行AbLang2・全採用クローン・cosine・n_neighbors = 15を固定", 11)
    shared._label(figure, font, 0.045, 0.934,
                  "min_distは検討用の値です。論文の設定値として確認されたものではありません。", 10)
    ...
    shared._label(figure, font, 0.045, 0.697,
                  ...
                  9, color="#53606c")
    for index, number in enumerate(info["min_dists"]):
        bottom = 0.486 - index * 0.214
        label = "現行のmin_dist" if number == 0.1 else "比較するmin_dist"
        shared._label(figure, font, 0.045, bottom + 0.183,
                      f"{chr(ord('B') + index)}  min_dist = {number}  ／  {label}", 15, weight="bold")
        contour = shared._density_row(figure, font, grids[f"md{number}_seed{seed}"],
...
def render(comparison_dir, reference_image, font_path):
    ...
    if metadata_path.exists() or list(folder.glob("min_dist_seed*_comparison.png")):
    ...
    seeds = info["seeds"]
    targets = {f"seed{seed}": folder / f"min_dist_seed{seed}_comparison.png" for seed in seeds}
    ...
    metadata = {
        ...
        "min_dists": info["min_dists"], "seeds": seeds,
        "fit_order_by_figure": {f"seed{seed}": [f"md{n}_seed{seed}" for n in info["min_dists"]]
                                for seed in seeds},
        "display": {
...
def main():
    ...
    try:
        ...
        print(f"Comparison rendering stopped ({type(error).__name__}); source files were not edited.",
              file=sys.stderr)
        return 1
    print("Two private min_dist comparison figures and their rendering record were saved.")
    return 0
