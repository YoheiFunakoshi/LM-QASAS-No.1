"""Render private UMAP n_epochs comparisons from frozen numerical artifacts.
...
def _checked_inputs(folder, reference, font_file):
    ...
    seeds, epochs = info.get("seeds"), info.get("epochs")
    if (not isinstance(seeds, list) or len(seeds) != 2 or len(set(seeds)) != 2
            ...
            or epochs != [200, 500, 1000]
            or any(type(number) is not int for number in epochs)):
        raise ValueError("Exactly two valid seeds and the predeclared epoch series are required.")
    ...
    expected_keys = {f"ep{number}_seed{seed}" for number in epochs for seed in seeds}
    fits = info.get("fits")
    if not isinstance(fits, dict) or set(fits) != expected_keys:
        raise ValueError("Every combination of the n_epochs series and both seeds is required.")
    ...
    if (not isinstance(frozen_parameters, dict) or frozen_parameters.get("n_epochs") != 200
            or projection.get("timepoint_counts") != counts
            or projection.get("kde", {}).get("bandwidth") != kde["bandwidth"]):
        raise ValueError("Counts, bandwidth and parameters must agree with the frozen baseline projection.")
    frozen_remaining = {name: value for name, value in frozen_parameters.items()
                        if name not in ("n_epochs", "random_state", "transform_seed")}
    ...
    for number in epochs:
        for seed in seeds:
            key = f"ep{number}_seed{seed}"
            fit = fits[key]
            if not isinstance(fit, dict) or fit.get("seed") != seed or fit.get("n_epochs") != number:
                raise ValueError("A fit does not match its declared n_epochs and seed.")
            parameters = fit.get("parameters")
            if (not isinstance(parameters, dict) or parameters.get("n_epochs") != number
                    or parameters.get("random_state") != seed or parameters.get("transform_seed") != seed
                    or parameters.get("n_neighbors") != 15 or parameters.get("metric") != "cosine"
                    or parameters.get("min_dist") != 0.1 or parameters.get("init") != "random"):
            ...
            if fixed_parameters is None:
                ...
                raise ValueError("UMAP settings other than n_epochs and seed must remain fixed.")
...
def _figure(info, grids, reference_pixels, font, seed, vmax):
    ...
    shared._label(figure, font, 0.045, 0.977, "UMAPの最適化回数による背景分布の比較", 21, weight="bold")
    shared._label(figure, font, 0.045, 0.955,
                  f"seed = {seed}  ／  現行AbLang2・全採用クローン・random・cosine・n_neighbors = 15・min_dist = 0.1を固定", 11)
    shared._label(figure, font, 0.045, 0.934,
                  "200・500・1,000回は検討用の値です。論文の設定値として確認されたものではありません。", 10)
    shared._label(figure, font, 0.045, 0.919,
                  "追加条件は最初から計算。200回の座標を延長したものではありません（基準座標は再利用）。", 9)
    ...
    shared._label(figure, font, 0.045, 0.697,
                  ...
                  9, color="#53606c")
    for index, number in enumerate(info["epochs"]):
        bottom = 0.486 - index * 0.214
        label = "現行の最適化回数" if number == 200 else "比較する最適化回数"
        shared._label(figure, font, 0.045, bottom + 0.183,
                      f"{chr(ord('B') + index)}  n_epochs = {number:,}  ／  {label}", 15, weight="bold")
        contour = shared._density_row(figure, font, grids[f"ep{number}_seed{seed}"],
...
def render(comparison_dir, reference_image, font_path):
    ...
    if metadata_path.exists() or list(folder.glob("epochs_seed*_comparison.png")):
    ...
    seeds = info["seeds"]
    targets = {f"seed{seed}": folder / f"epochs_seed{seed}_comparison.png" for seed in seeds}
    ...
    metadata = {
        ...
        "epochs": info["epochs"], "seeds": seeds,
        "fit_order_by_figure": {f"seed{seed}": [f"ep{n}_seed{seed}" for n in info["epochs"]]
                                for seed in seeds},
        "display": {
...
def main():
    ...
    try:
        ...
        print(f"Comparison rendering stopped ({type(error).__name__}); source files were not edited.",
              file=sys.stderr)
        return 1
    print("Two private n_epochs comparison figures and their rendering record were saved.")
    return 0
